	...
	def save_model(self, request, obj, form, change):
		...
		for x in StageGroup.objects.filter(group=obj.group).order_by('order'):
			...
			if x.order == 1 and len(a)<1:
				UserStage.objects.create(user=obj.user, stage=x.stage, order=x.order, sessions_completed=0, status=1, start_date=datetime.datetime.now(), last_session_completed=datetime.datetime.now())
			elif len(a)<1:
				UserStage.objects.create(user=obj.user, stage=x.stage, order=x.order, sessions_completed=0, status=2)
			else:
				pass
		obj.save()
